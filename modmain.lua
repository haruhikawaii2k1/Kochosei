...
local function spawntom(inst, data)
	if data.name == "Spawn_Tom" then
		if inst.components.container:IsFull() then
			return
		end

		local has_hoasen = false
		local getallitem = inst.components.container:ReferenceAllItems()

		for _, item in ipairs(getallitem) do
			if item:HasTag("hoasen") then
				has_hoasen = true
				inst.components.container:RemoveItem(item, false) -- Xóa item có tag "hoasen"
				break -- Chỉ xóa 1 cái
			end
		end
		if has_hoasen and not inst.components.container:IsFull() then
			...
			inst.components.timer:StopTimer("Spawn_Tom")
		end
	end
end
...
AddPrefabPostInit("fish_box", function(inst)
	...
	inst.components.container.onclosefn = function(inst, data)
		...
		if
			findtem
			and not inst.components.timer:TimerExists("Spawn_Tom")
